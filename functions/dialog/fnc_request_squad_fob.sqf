...
if (commandpointsblu1 >= _cost) then {
	...
	if (_task_force == true) then {
		...
		// Define ranks for the group
		_ranks = [];
		_ranks set [0, "SERGEANT"]; // Leader rank

		// Add at least two corporals
		if (count _grouptype > 1) then { _ranks set [1, "CORPORAL"]; };
		if (count _grouptype > 2) then { _ranks set [2, "CORPORAL"]; };

		// Fill the remaining slots with PRIVATES
		for "_i" from (count _ranks) to ((count _grouptype) - 1) do {
			_ranks pushBack "PRIVATE";
		};

		// Ensure the rank array matches the group type array size
		if ((count _ranks) != (count _grouptype)) then {
			diag_log "Error: Rank array size does not match group type array size!";
		};
		
		_group = [_spawnPos, WEST, _grouptype, [], _ranks, blufor_ai_skill] call BIS_fnc_spawnGroup;
