...
// add event handler for unit insignia
addMissionEventHandler ["EntityCreated", {
    params ["_entity"];
    if ((typeOf _entity isKindOf "Man") && (side _entity == west)) then {
...
_hq setRank "COLONEL";
[_hq, "111thID"] call BIS_fnc_setUnitInsignia;
...
if (VERSION_X == "MODERN") then {
	if !(("OPTRE" in BLUFOR_FactionFile_X) 
		...
		removeheadgear _hq;
		_hq addHeadgear "H_Beret_Colonel";
	};
};
